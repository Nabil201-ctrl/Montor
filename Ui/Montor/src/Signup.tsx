...
const Signup = () => {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const res = await auth.register({ email, password, name }) as any
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-bg relative overflow-hidden px-6 py-12">
      ...
      <div className="w-full max-w-md z-10">
        <div className="text-center mb-10">
          ...
          <h1 className="text-4xl font-black tracking-tight text-text mb-3">Create Account</h1>
          <p className="text-text-secondary">Start tracking your momentum today.</p>
        </div>
        ...
        <div className="bg-bg-card border border-border rounded-radius p-8 shadow-[0_20px_50px_rgba(0,0,0,0.3)] backdrop-blur-xl relative group">
          ...
          <form onSubmit={handleSignup} className="space-y-4 relative z-10">
            ...
            <div>
              <label className="block text-[10px] font-mono text-text-muted uppercase tracking-widest mb-1.5 ml-1">Full Name</label>
              <input
                type="text"
                value={name}
            ...
            <div>
              <label className="block text-[10px] font-mono text-text-muted uppercase tracking-widest mb-1.5 ml-1">Password</label>
              <input
                type="password"
                value={password}
